...
describe('getRecaptchaToken', () => {
  ...
  it('calls execute with the provided site key and default action', async () => {
    ...
    expect(mockExecute).toHaveBeenCalledWith('my-site-key', { action: 'submit' })
  })
  ...
  it('calls execute with a custom action when provided', async () => {
    ...
    expect(mockExecute).toHaveBeenCalledWith('my-site-key', { action: 'cloud_credits_submit' })
  })
  ...
  it('rejects if execute throws', async () => {
    ...
    await expect(getRecaptchaToken('site-key')).rejects.toThrow('reCAPTCHA failed')
  })
})